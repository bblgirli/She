...
self.addEventListener('push',event=>{
 let data={};try{data=event.data?event.data.json():{}}catch(_){try{data=JSON.parse(event.data?.text?.()||'{}')}catch(_){}}
 if(data.type!=='incoming_call'&&data.type!=='new_message')return;
 const incoming=data.type==='incoming_call';
 const conversationId=data.conversationId||'';
 const tag=incoming?`incoming-call-${data.callId||Date.now()}`:`message-${conversationId||'general'}`;
 ...
 event.waitUntil(self.registration.showNotification(title,{body,icon:'/Img_9610(1).png',badge:'/Img_9610(1).png',tag,renotify:false,requireInteraction:incoming,vibrate:incoming?[300,100,300,100,500]:[200],data:{type:data.type,conversationId,callId:data.callId||'',callerId:data.callerId||'',callerName:data.callerName||'Contact'}}));
});
